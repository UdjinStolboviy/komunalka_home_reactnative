...
export const FlatListItemView = (props: FlatListItemViewProps) => {
  const massage = `
   ...
   ${props.item.resultElectricity} * ${props.item.electricityTariff} грн = ${
    props.item.multiplicationElectricity
  } грн 

   Вода: ${props.item.resultWater} куб.м
   ${props.item.messageWater}
   ${props.item.waterTariff} грн за куб.м
   ${props.item.resultWater} * ${props.item.waterTariff} грн = ${
    props.item.multiplicationWater
  } грн 
   ...
   Всього:  ${props.item.multiplicationElectricity} + ${
    props.item.multiplicationWater
  } + 
    ${props.item.resultInternet} + ${props.item.garbageRemovalTariff} + 
    ${props.item.resultRent} + ${props.item.resultOtherOptions} = ${Math.ceil(
    props.item.resultAllCalculate,
  )} грн 
`;
